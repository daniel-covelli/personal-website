  ...
  return (
    <section className="mt-8 rounded-lg border border-gray-100 bg-white p-6 shadow-sm">
      ...
      <details className="group mt-4 rounded-lg border border-gray-200 bg-gray-50">
        <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-2.5 text-sm font-medium text-gray-700">
        ...
        <div className="border-t border-gray-200 px-4 py-3">
          <p className="mb-2 text-xs text-gray-500">{previewDescription}</p>
          {preview.status === 'error' ? (
            <p className="rounded-md bg-red-50 px-3 py-2 font-mono text-xs text-red-700">
              {preview.message}
            </p>
          ) : (
            <pre className="max-h-96 overflow-auto whitespace-pre-wrap break-words rounded-md bg-white p-3 font-mono text-xs leading-relaxed text-gray-700 ring-1 ring-inset ring-gray-200">
              {preview.text}
            </pre>
          )}
        </div>
