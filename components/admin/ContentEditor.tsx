...
export default function ContentEditor({ initialContent }: ContentEditorProps) {
  ...
  function updateEducation(index: number, field: keyof Education, value: string | string[]) {
  ...
  function addEducationBullet(eduIndex: number) {
    const newEdu = [...content.education];
    newEdu[eduIndex] = { ...newEdu[eduIndex], bullets: [...(newEdu[eduIndex].bullets || []), ''] };
    setContent({ ...content, education: newEdu });
  }

  function updateEducationBullet(eduIndex: number, bulletIndex: number, value: string) {
    const newEdu = [...content.education];
                  ...
                  {(edu.bullets || []).map((bullet, bulletIndex) => (
                    <div key={bulletIndex} className="flex gap-2">
                      <input
                        ...
                        placeholder={`Bullet ${bulletIndex + 1}`}
                        value={bullet}
                        onChange={(e) => updateEducationBullet(index, bulletIndex, e.target.value)}
                        className="flex-1 px-3 py-2 border border-gray-300 rounded-lg"
                      />
                      <button
                        onClick={() => removeEducationBullet(index, bulletIndex)}
                        className="px-3 py-2 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg"
                      >
                        ×
                      </button>
